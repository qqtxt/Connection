...
class Connection
{
    ...
    */
    ...
    {
        try {
			if(defined('APP_DEBUG') && APP_DEBUG) file_put_contents('jetee.txt',$query.' [] '.print_r($parameters,1)."\r\n",FILE_APPEND);
            $this->sQuery = @$this->pdo->prepare($query);
            if($parameters)
				$this->bindMore($parameters);
			#if(defined('APP_DEBUG') && APP_DEBUG) file_put_contents('jetee.txt',$query.' [] '.print_r($this->parameters,1)."\r\n",FILE_APPEND);
    ...
    public function count($query = '', $params = null){
		$this->select('count(*)');
		return $this->single($query = '', $params = null);
	}
